...
function simulate() {
	...
	if(idata == Ndata) {
		...
		// Display mass position of caOutvas
		clearCanvas();
		drawMassOnCanvas(x1, y1, 0.5 * D1, cL1, cF1, caOut);
		drawMassOnCanvas(x2, y2, 0.5 * D2, cL2, cF2, caOut);
...
function drawMassOnCanvas (x, y, R, cLine , cFill , caOut) {
	...
	// Transform x from real coordinate to caOutvas coordinate
	function tx(x) {
	...
	// Transform y from real coordinate to caOutvas coordinate
	function ty(y) {
...
function setElementsLayout(){
	...
	// Create output caOutvas
	caOut = document.createElement("caOutvas");
	caOut.width = "400";
	caOut.height = "400";
...
function buttonClick() {
	...
	if(cap == "Load") {
		...
		tout(taOut1, "Wafa Fatimah Rastiadi 10215048"
		);
	}
}
